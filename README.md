...
| -------------------------- | ------------------------------------------------------------------------------------------------------------ | ---------------------------------------- |
| `single_writer`            | A lone thread can take `LOCK_EX`, do work, and release it without error.                                     |                                          |
| `multiple_readers`         | Five helpers grab \`LOCK\_SH                                                                                 |  LOCK\_NB\` concurrently; no one blocks. |
| `writer_excludes_reader`   | Parent holds `LOCK_EX`; a non‑blocking reader (ELF 8) must fail with `EWOULDBLOCK`.                          |                                          |
| `writer_excludes_writer`   | Parent holds `LOCK_EX`; another thread’s \`LOCK\_EX                                                          |  LOCK\_NB\` must fail immediately.       |
| `reader_excludes_writer`   | Parent holds `LOCK_SH`; a non‑blocking writer must fail.                                                     |                                          |
| `queued_writer_does_block` | Reader‑1 holds `LOCK_SH`, a writer queues, then Reader‑2 tries and *must wait* until the queued writer runs. |                                          |
| `bad_fd`                   | `flock()` on an invalid FD correctly returns error.                                                          |                                          |
| `double_unlock`            | Unlocking twice in a row on the same descriptor returns error the second time.                               |                                          |
| `upgrade_flock`            | `LOCK_SH` can be upgraded to `LOCK_EX`; behaviour with active readers is validated.                          |                                          |
| `downgrade_flock`          | `LOCK_EX` can be downgraded to `LOCK_SH`; queued readers are released and data stays intact.                 |                                          |
