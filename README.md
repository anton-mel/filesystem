...
There are two producer and two consumer processes launched in `kern/init.c`, along with an idle process for each CPU. This results in a total of **six processes** that the scheduler preempts and manages over 2 CPUs. I aimed to optimize the system by putting the idle thread to sleep until the producer-consumer tasks finished, but due to limitations in the OS (aka, lack of proper synchronization and wake-up hooks for the userspace), I’ve deferred this optimization for future work.
